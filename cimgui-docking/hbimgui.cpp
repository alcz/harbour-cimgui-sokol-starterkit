...
HB_FUNC( IMGUILISTCLIPPER_FORCEDISPLAYRANGEBYINDICES )
{
   ImGuiListClipper* pOut = ( ImGuiListClipper* ) hb_parptr( 1 );
   int item_min = hb_parni( 2 );
   int item_max = hb_parni( 3 );
   ImGuiListClipper_ForceDisplayRangeByIndices(pOut,item_min,item_max);
}
...
{
   ImGuiTooltipFlags tooltip_flags = ( ImGuiTooltipFlags ) hb_parni( 1 );
   ImGuiWindowFlags extra_window_flags = ( ImGuiWindowFlags ) hb_parni( 2 );
...
HB_FUNC( IGBRINGWINDOWTODISPLAYBEHIND )
{
   ImGuiWindow* window = ( ImGuiWindow* ) hb_parptr( 1 );
   ImGuiWindow* above_window = ( ImGuiWindow* ) hb_parptr( 2 );
...
HB_FUNC( IGCALCROUNDINGFLAGSFORRECTINRECT )
{
   PHB_ITEM pr_in = hb_param( 1, HB_IT_ARRAY );
   const ImRect r_in = ImRect{ ImVec2{ _paf( pr_in, 1 ), _paf( pr_in, 2 ) }, ImVec2{ _paf( pr_in, 3 ), _paf( pr_in, 4 ) } };
   PHB_ITEM pr_outer = hb_param( 2, HB_IT_ARRAY );
   const ImRect r_outer = ImRect{ ImVec2{ _paf( pr_outer, 1 ), _paf( pr_outer, 2 ) }, ImVec2{ _paf( pr_outer, 3 ), _paf( pr_outer, 4 ) } };
   float threshold = ( float ) hb_parnd( 3 );
   ImDrawFlags ret = igCalcRoundingFlagsForRectInRect(r_in,r_outer,threshold);
   hb_retni( ( int ) ret );
}
...
HB_FUNC( IGDEBUGNODEWINDOWSLISTBYBEGINSTACKPARENT )
{
   ImGuiWindow** windows = ( ImGuiWindow** ) hb_parptr( 1 );
   int windows_size = hb_parni( 2 );
   ImGuiWindow* parent_in_begin_stack = ( ImGuiWindow* ) hb_parptr( 3 );
...
HB_FUNC( IGDOCKNODEISINHIERARCHYOF )
{
   ImGuiDockNode* node = ( ImGuiDockNode* ) hb_parptr( 1 );
   ImGuiDockNode* parent = ( ImGuiDockNode* ) hb_parptr( 2 );
   bool ret = igDockNodeIsInHierarchyOf(node,parent);
   hb_retl( ret );
}
...
HB_FUNC( IGFINDBOTTOMMOSTVISIBLEWINDOWWITHINBEGINSTACK )
{
   ImGuiWindow* window = ( ImGuiWindow* ) hb_parptr( 1 );
   ImGuiWindow* ret = igFindBottomMostVisibleWindowWithinBeginStack(window);
   hb_retptr( ( void * ) ret );
}
...
HB_FUNC( IGFINDWINDOWDISPLAYINDEX )
{
   ImGuiWindow* window = ( ImGuiWindow* ) hb_parptr( 1 );
   int ret = igFindWindowDisplayIndex(window);
   hb_retni( ret );
}
...
HB_FUNC( IGGETMOUSECLICKEDCOUNT )
{
   ImGuiMouseButton button = ( ImGuiMouseButton ) hb_parni( 1 );
   int ret = igGetMouseClickedCount(button);
   hb_retni( ret );
}
...
HB_FUNC( IGGETTOPMOSTANDVISIBLEPOPUPMODAL )
{
   ImGuiWindow* ret = igGetTopMostAndVisiblePopupModal();
   hb_retptr( ( void * ) ret );
}
...
HB_FUNC( IGIMISFLOATABOVEGUARANTEEDINTEGERPRECISION )
{
   float f = ( float ) hb_parnd( 1 );
   bool ret = igImIsFloatAboveGuaranteedIntegerPrecision(f);
   hb_retl( ret );
}
...
HB_FUNC( IGIMQSORT )
{
   void* base = ( void* ) hb_parptr( 1 );
   size_t count = ( size_t ) hb_parnl( 2 );
   size_t size_of_element = ( size_t ) hb_parnl( 3 );
   igImQsort(base,count,size_of_element,&hb_ig_compare_func);
}
...
HB_FUNC( IGISWINDOWWITHINBEGINSTACKOF )
{
   ImGuiWindow* window = ( ImGuiWindow* ) hb_parptr( 1 );
   ImGuiWindow* potential_parent = ( ImGuiWindow* ) hb_parptr( 2 );
   bool ret = igIsWindowWithinBeginStackOf(window,potential_parent);
   hb_retl( ret );
}
...
{
   ImGuiNavItemData* result = ( ImGuiNavItemData* ) hb_parptr( 1 );
...
HB_FUNC( IGSCROLLBAREX )
{
   ...
   ImGuiID id = ( ImGuiID ) hb_parni( 2 );
   ImGuiAxis axis = ( ImGuiAxis ) hb_parni( 3 );
   ImS64 /* @ */ _p_scroll_v = ( HB_MAXINT ) hb_parnint( 4 );
   ImS64 * p_scroll_v = &_p_scroll_v;
   ImS64 avail_v = ( HB_MAXINT ) hb_parnint( 5 );
   ImS64 contents_v = ( HB_MAXINT ) hb_parnint( 6 );
   ImDrawFlags flags = ( ImDrawFlags ) hb_parni( 7 );
   bool ret = igScrollbarEx(bb,id,axis,p_scroll_v,avail_v,contents_v,flags);
   hb_itemPutNInt( hb_paramError( 4 ), _p_scroll_v );
   hb_retl( ret );
}
...
HB_FUNC( IGSPLITTERBEHAVIOR )
{
   ...
   ImU32 bg_col = ( HB_U32 ) hb_parnl( 10 );
   bool ret = igSplitterBehavior(bb,id,axis,size1,size2,min_size1,min_size2,hover_extend,hover_visibility_delay,bg_col);
...
HB_FUNC( IGWINDOWRECTABSTOREL )
{
   PHB_ITEM pOutItem = hb_param( 1, HB_IT_ANY );
   ImRect pOut;
   ImGuiWindow* window = ( ImGuiWindow* ) hb_parptr( 2 );
   PHB_ITEM pr = hb_param( 3, HB_IT_ARRAY );
   const ImRect r = ImRect{ ImVec2{ _paf( pr, 1 ), _paf( pr, 2 ) }, ImVec2{ _paf( pr, 3 ), _paf( pr, 4 ) } };
   igWindowRectAbsToRel(&pOut,window,r);
   _ImRecttoA( &pOut, pOutItem );
}

/* void igWindowRectRelToAbs(ImRect *pOut,ImGuiWindow* window,const ImRect r) */
HB_FUNC( IGWINDOWRECTRELTOABS )
{
   PHB_ITEM pOutItem = hb_param( 1, HB_IT_ANY );
   ImRect pOut;
   ImGuiWindow* window = ( ImGuiWindow* ) hb_parptr( 2 );
...
static void s_ImGuiListClipper_getTempData( ImGuiListClipper * p )
{
   void* ret = p->TempData;
   hb_retptr( ( void * ) ret );
}

static void(*s_ImGuiListClipper_fields[])( ImGuiListClipper * ) = { s_ImGuiListClipper_getDisplayStart, s_ImGuiListClipper_getDisplayEnd, s_ImGuiListClipper_getItemsCount, s_ImGuiListClipper_getItemsHeight, s_ImGuiListClipper_getStartPosY, s_ImGuiListClipper_getTempData };
...
static void s_ImGuiIO_setConfigDockingWithShift( ImGuiIO * p )
{
   p->ConfigDockingWithShift = hb_parl( 3 );
}
...
static void s_ImGuiIO_getConfigDockingWithShift( ImGuiIO * p )
{
   bool ret = p->ConfigDockingWithShift;
   hb_retl( ret );
}
...
static void s_ImGuiIO_getMouseClickedCount( ImGuiIO * p )
{
   ImU16 ret;
   HB_SIZE pos = hb_parns( 3 );
   if( pos == 0 || pos > 5 )
      return;
   ret = p->MouseClickedCount[ pos - 1 ];
}

static void s_ImGuiIO_getMouseClickedLastCount( ImGuiIO * p )
{
   ImU16 ret;
   HB_SIZE pos = hb_parns( 3 );
   if( pos == 0 || pos > 5 )
      return;
   ret = p->MouseClickedLastCount[ pos - 1 ];
}
