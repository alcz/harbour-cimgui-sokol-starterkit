...
{
   const char* label = hb_parcx( 1 );
   ImS64 /* @ */ _flags = ( HB_MAXINT ) hb_parnint( 2 );
   ImS64 * flags = &_flags;
   ImS64 flags_value = ( HB_MAXINT ) hb_parnint( 3 );
   bool ret = igCheckboxFlagsS64Ptr(label,flags,flags_value);
   hb_itemPutNInt( hb_paramError( 2 ), _flags );
   hb_retl( ret );
}
...
HB_FUNC( IGLISTBOXHEADERINT )
{
   const char* label = hb_parcx( 1 );
   int items_count = hb_parni( 2 );
   int height_in_items = hb_parnidef( 3, -1 );
